import { NextRequest, NextResponse } from 'next/server';
import { currentUser } from '@clerk/nextjs/server';
import prisma from '@/lib/db';

// Mark as dynamic to prevent static analysis issues
export const dynamic = 'force-dynamic';

export async function POST(req: NextRequest) {
  try {
    const user = await currentUser();
    
    if (!user) {
      return new NextResponse("Unauthorized", { status: 401 });
    }
    
    const body = await req.json();
    const { formId, action } = body;
    
    if (!formId) {
      return new NextResponse("Form ID is required", { status: 400 });
    }
    
    if (!action || !['trash', 'restore'].includes(action)) {
      return new NextResponse("Valid action (trash or restore) is required", { status: 400 });
    }
    
    // First, find the internal user ID from the external Clerk ID
    const dbUser = await prisma.user.findUnique({
      where: { externalId: user.id },
      select: { id: true }
    });
    
    if (!dbUser) {
      return new NextResponse("User not found", { status: 404 });
    }
    
    // Check if the form exists and belongs to the user
    const form = await prisma.form.findFirst({
      where: { 
        id: formId,
        userId: dbUser.id,
        isDeleted: false 
      }
    });
    
    if (!form) {
      return new NextResponse("Form not found or unauthorized", { status: 404 });
    }
    
    // Update the form's trash status
    const updatedForm = await prisma.form.update({
      where: { id: formId },
      data: { 
        isInTrash: action === 'trash' ? true : false,
        // When restoring, update the updatedAt timestamp
        ...(action === 'restore' && { updatedAt: new Date() })
      }
    });
    
    return NextResponse.json({ 
      success: true, 
      action,
      form: updatedForm
    });
  } catch (error) {
    console.error("[TRASH_TOGGLE]", error);
    return new NextResponse("Internal Error", { status: 500 });
  }
} 